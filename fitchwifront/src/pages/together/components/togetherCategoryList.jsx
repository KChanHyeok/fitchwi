...
const TogetherCategoryList = ({ togetherList }) => {
  ...
  return (
    <Stack sx={{ width: 1000, height: 800, margin: "auto" }} flex={7} p={3}>
      ...
      {togetherList.filter((data) => (data.togetherState !== "삭제신청중" && data.togetherCategory === categorys[togetherCategoryText]  && data.togetherState!=="결제완료")).length === 0 ? (
        <Typography textAlign="center" height={100} lineHeight={40}>
          😀 현재 진행중인 함께해요가 없습니다
        </Typography>
      ) : (
        togetherList
          .filter((data) => data.togetherState !== "삭제신청중" && data.togetherCategory === categorys[togetherCategoryText]  && data.togetherState!=="결제완료")
